...
namespace SGP.Core.Application.Controllers
{
    public class PublicacionesController : Controller
    {
        ...
        public PublicacionesController(ValidateUserSession validateUserSession, IPublicacionService publicacionService, IComentarioService comentarioService)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> CreatePublicacion(PublicacionViewModel vm)
        {
            if (!_validateUserSession.HasUser())
            {
                return RedirectToRoute(new { controller = "Login", action = "Index" });
        ...
        [HttpPost]
        public async Task<IActionResult> EditPublicacion(PublicacionViewModel vm)
        {
            if (!_validateUserSession.HasUser())
            {
                return RedirectToRoute(new { controller = "Login", action = "Index" });
            ...
            {
                await _publicacionService.Update(vm);
        ...
        public async Task<IActionResult> DeletePublicacion(int IdPublicacion)
        {
            ...
            await _publicacionService.Delete(IdPublicacion);
